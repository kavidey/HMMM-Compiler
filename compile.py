...
def parse_binary_op(node: pycparser.c_ast.BinaryOp, program: HmmmProgram) -> None:
    ...
    assert (
        len(used_registers) < 11
    ), "Cannot use more than 10 variables in a single expression"
    unused_registers = find_unused_registers(
        used_registers.union(
            {HmmmRegister.R0, HmmmRegister.R13, HmmmRegister.R14, HmmmRegister.R15}
        )
    )
    ...
    if isinstance(node.left, pycparser.c_ast.ID):
        ...
        program.add_instruction(
            generate_instruction("copy", left_register, HmmmRegister.R13)
        )
    ...
    if isinstance(node.right, pycparser.c_ast.ID):
        ...
        program.add_instruction(
            generate_instruction("copy", right_register, HmmmRegister.R13)
        )
        program.add_instruction(generate_instruction("popr", HmmmRegister.R13))
    ...
    if node.op == "+":
        program.add_instruction(
            generate_instruction("add", HmmmRegister.R13, left_register, right_register)
        )
    elif node.op == "-":
        program.add_instruction(
            generate_instruction("sub", HmmmRegister.R13, left_register, right_register)
        )
    elif node.op == "*":
        program.add_instruction(
            generate_instruction("mul", HmmmRegister.R13, left_register, right_register)
        )
    elif node.op == "/":
        program.add_instruction(
            generate_instruction("div", HmmmRegister.R13, left_register, right_register)
        )
    elif node.op == "%":
        program.add_instruction(
            generate_instruction("mod", HmmmRegister.R13, left_register, right_register)
        )
...
def parse_unary_op(node: pycparser.c_ast.UnaryOp, program: HmmmProgram) -> None:
    ...
    if isinstance(node.expr, pycparser.c_ast.ID):
        program.add_instruction(
            generate_instruction("copy", HmmmRegister.R13, HmmmRegister(node.expr.name))
        )
    elif isinstance(node.expr, pycparser.c_ast.Constant):
        program.add_instruction(
            generate_instruction("setn", HmmmRegister.R13, int(node.expr.value))
        )
    ...
    if node.op == "-":
        program.add_instruction(
            generate_instruction(
                "sub", HmmmRegister.R13, HmmmRegister.R0, HmmmRegister.R13
            )
        )
    elif node.op == "p--":
...
def parse_decl_assign(
    ...
    if isinstance(val, pycparser.c_ast.Constant):
        ...
        program.add_instruction(
            generate_instruction("setn", HmmmRegister(varname), int(val.value))
        )
    elif isinstance(val, pycparser.c_ast.BinaryOp):
        program.add_instruction(generate_instruction("pushr", HmmmRegister.R13))
        parse_binary_op(val, program)
        program.add_instruction(
            generate_instruction("copy", HmmmRegister(varname), HmmmRegister.R13)
        )
        program.add_instruction(generate_instruction("popr", HmmmRegister.R13))
    elif isinstance(val, pycparser.c_ast.ID):
        program.add_instruction(
            generate_instruction("copy", HmmmRegister(varname), HmmmRegister(val.name))
        )
    # Test this later
...
def parse_condition(
...
) -> Tuple[HmmmInstruction, List[HmmmInstruction]]:
    ...
    used_registers = find_used_registers(node)
    assert (
        len(used_registers) < 12
    ), "Cannot use more than 11 variables in a single expression"
    unused_registers = find_unused_registers(
        used_registers.union(
            {HmmmRegister.R0, HmmmRegister.R13, HmmmRegister.R14, HmmmRegister.R15}
        )
    ...
    if isinstance(node.left, pycparser.c_ast.ID):
        program.add_instruction(
            generate_instruction("copy", left_register, HmmmRegister(node.left.name))
        )
    elif isinstance(node.left, pycparser.c_ast.Constant):
        program.add_instruction(
    ...
    if isinstance(node.right, pycparser.c_ast.Constant) and node.right.value != "0":
        ...
        program.add_instruction(
            generate_instruction(
                "sub", left_register, left_register, HmmmRegister(node.right.name)
            )
        )
...
def parse_for(node: pycparser.c_ast.While, program: HmmmProgram) -> None:
    ...
    program.add_instruction(
        generate_instruction(
            "copy", HmmmRegister(node.next.expr.name), HmmmRegister.R13
        )
    )
    program.add_instruction(generate_instruction("popr", HmmmRegister.R13))
        ...
        if isinstance(stmt, pycparser.c_ast.Decl) or isinstance(
            ...
            if stmt.name.name == "printf":
                ...
                if isinstance(stmt.args.exprs[1], pycparser.c_ast.ID):
                    program.add_instruction(
                        generate_instruction(
                            "write", HmmmRegister(stmt.args.exprs[1].name)
                        )
                    )
                # If the argument is a constant
                elif isinstance(stmt.args.exprs[1], pycparser.c_ast.Constant):
                    program.add_instruction(
                        generate_instruction("pushr", HmmmRegister.R13)
                    )
                    program.add_instruction(
                        generate_instruction(
                            "setn", HmmmRegister.R13, int(stmt.args.exprs[1].value)
                        )
                    )
                    program.add_instruction(
                        generate_instruction("write", HmmmRegister.R13)
                    )
                    program.add_instruction(
                        generate_instruction("popr", HmmmRegister.R13)
                    )
                ...
                program.add_instruction(
                    generate_instruction(
                        "read", HmmmRegister(stmt.args.exprs[1].expr.name)
                    )
                )
            ...
            program.add_instruction(
                generate_instruction(
                    "copy", HmmmRegister(stmt.expr.name), HmmmRegister.R13
                )
            )
            program.add_instruction(generate_instruction("popr", HmmmRegister.R13))
